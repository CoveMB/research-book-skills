...
When the user explicitly asks for JSON or a contract artifact, use `shared/contracts/book/book_artifact.schema.json` with `artifact_type: chapter_brief`. If the output is normal Markdown, do not force the JSON contract.
...
- None by default.
- May create or update user-requested chapter briefs, outlines, or planning notes in the user-designated project or workspace.
