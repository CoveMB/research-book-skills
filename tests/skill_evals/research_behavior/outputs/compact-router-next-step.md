# Research route
Selected skill: research-intent-router.

Source basis: broad chapter idea only.
How to use this result: TRIAGE ONLY - Use this only to choose the next review step; do not treat it as source verification.
Mode: normal mode

| Best route | Why this skill | Lookup needed? | Still unverified |
| --- | --- | --- | --- |
| scholarly-research-agenda | scope is not stable yet | no | question, audience, and evidence plan |

Next action: Draft the research question and scope boundary.
